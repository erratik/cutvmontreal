...
class WatchLaterView extends WatchLaterModel {
	...
	public function displayView() {
		...
		$model = $this;
		$this->watchLaterDetails = $model->get_watch_video_details($userId);
		$this->watchLaterVideosStatus = $model->get_videos_status($userId);
		$this->watchLaterCount   = $model->get_watch_video_count($userId);
		$this->showTemplate();
	}
	/**
	...
	public function showTemplate() {
		...
		if(count($this->watchLaterDetails) > 0 ) {
			$output .='<div class="watchButtonContainer"><button class="clearButton watchButton" id="clearButton" title="'.__( 'Clear Watch History', APPTHA_VGALLERY ).'">'.__( 'Clear Watch History', APPTHA_VGALLERY ).'</button></div>';
		}
