...
trait WatchHelper {
	 ...
	 */
	public function get_watch_video_details($userId) {
		global $wpdb;
		$videoArray = $wpdb->get_col($wpdb->prepare("SELECT vid FROM ".$this->watchDetailsTable." WHERE userid=%d ORDER BY date DESC",$userId));
		if(!empty($videoArray)) {
			$videoResults = $wpdb->get_results("SELECT * FROM ".$this->hdvideoshareTable." WHERE vid IN (".implode(',',$videoArray).") ORDER BY field(vid,".implode(',',$videoArray).") LIMIT 10 OFFSET 0");
		}
	 ...
	 */
	public function get_watch_video_count($userId) {
		global $wpdb;
		$videoArray = $wpdb->get_col($wpdb->prepare("SELECT vid FROM ".$this->watchDetailsTable." WHERE userid=%d ORDER BY date DESC",$userId));
		if(!empty($videoArray)) {
			$videoResults = $wpdb->get_results("SELECT * FROM ".$this->hdvideoshareTable." WHERE vid IN (".implode(',',$videoArray).")");
		}
	 ...
	 */
	public function getWatchLaterVideoIds($userId) {
		global $wpdb;
		$result = $wpdb->get_col($wpdb->prepare("SELECT vid FROM ".$this->watchDetailsTable." WHERE userid=%d AND status=%d",$userId,1));
		return $result;
	}
	/**
	 ...
	 * @return string
	 */
	public function getVideosHelper($viewName) {
		$videoOutput='';
		foreach($this->videoDetails as $videoDetails) {
            ...
            <span class="ratethis1 '.$this->ratearray[$videoStar].' watchContentRate"></span>
            </p>
            </div>
			...
			if($viewName == 'playlist') {
				$videoOutput.='<span class="watchVideoCloseButton" onclick="clearPlaylistVideo(this,'.$videoDetails->vid.')"><img src="'.$this->closeButtonPath.'" title="Delete video"></span>';
			}
            else {
            	$videoOutput.='<span class="watchVideoCloseButton" onclick="clearSingleVideo(this,'.$videoDetails->vid.')"><img src="'.$this->closeButtonPath.'" title="Delete video"></span>';
            }
            $videoOutput.='</div>';
		}
		return $videoOutput;
	 ...
	 * @return string
	 */
	public function getVideoByAjax($videoResults,$statusArray='') {
		foreach($videoResults as $videoDetail) {
			...
			if(empty($statusArray)) {
				$result[] = array('vid' => $videoDetail->vid,'name' =>$videoName,'url' => $videoUrl,'hitcount' =>$videoHitcount,'rate' => $this->ratearray[$videoStar],'thumb' => $thumbImage,'description' =>$videoDescription);
			}
			else {
				$result[] = array('vid' => $videoDetail->vid,'name' =>$videoName,'url' => $videoUrl,'hitcount' =>$videoHitcount,'rate' => $this->ratearray[$videoStar],'thumb' => $thumbImage,'description' =>$videoDescription,'status' =>(in_array($videoDetail->vid,$statusArray)) ? 1 : -2);
			}
		}
		return $result;
	}
