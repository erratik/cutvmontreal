<?php
	...
	global $cutv_timezones , $cutv_countries , $cutv_colors , $cutv_roles;
	global $cutv_services , $cutv_status , $cutv_private_cpt;
	global $cutv_types , $cutv_types_;
	global $cutv_days_names , $cutv_days , $cutv_hours , $cutv_days_of_month;
	global $cutv_base_definitions;
	//Dynamic Global Data SDD
	global $cutv_dynamics , $cutv_options , $cutv_cron_token , $cutv_tokens , $cutv_stress , $cutv_activation;
	global $cutv_addons , $cutv_fillers , $cutv_filler_data , $cutv_all_plugins , $cutv_setters;
	global $cutv_notices , $cutv_deferred , $cutv_deferred_ids , $cutv_imported;

	global $cutv_unwanted , $cutv_unwanted_ids ;

	global $cutv_getmb_unsupported_themes ;

	// Making those variables global
	global $current_user , $cutv_pages , $cutv_rating_levels;

	global $cutv_vs , $cutv_act ;

	global $default_fetching_options , $default_videosFound;
		
	global $service_not_enabled ;
	global $cutv_timer_id ;
	/**************************    STATIC *****************************************************/
	$service_not_enabled = ' <div class = "cutv_source_links cutv_service_disabled">SERVICE NOT ENABLED</div> ';
	
	$cutv_getmb_unsupported_themes = array(
		'newstube',
	);

	$cutv_act = cutv_get_activation('cutv');
	...
	/* New Video Services */
	//$cutv_vs = array();
	//$cutv_vs = apply_filters('cutv_extend_video_services', $cutv_vs);
	...
	//$cutv_services = apply_filters( 'cutv_extend_video_services' , $cutv_services );
	...
	$cutv_types_ = array(
		'search'      => array(
			'label' => 'Search' ,
			'icon'  => 'fa-search' ,
			'color' => $cutv_services[ 'youtube' ][ 'color' ] ,
		) ,
		'search_vo'   => array(
			'label' => 'Search' ,
			'icon'  => 'fa-search' ,
			'color' => $cutv_services[ 'vimeo' ][ 'color' ] ,
		) ,
		'search_dm'   => array(
			'label' => 'Search' ,
			'icon'  => 'fa-search' ,
			'color' => $cutv_services[ 'dailymotion' ][ 'color' ] ,
		) ,
		'trendy'      => array(
			'label' => 'Trends' ,
			'icon'  => 'fa-trophy' ,
	...
	/* dataFiller Names */
	$cutv_filler_data = array(
		'cutv_video_id'                => __( 'Video ID' , WPVR_LANG ) ,
		'cutv_dynamic_views'           => __( 'Dynamic Video Views' , WPVR_LANG ) ,
		'cutv_video_service'           => __( 'Video Service' , WPVR_LANG ) ,
		'cutv_video_service_url'       => __( 'Video URL' , WPVR_LANG ) ,
		'cutv_video_service_url_https' => __( 'Video URL (https)' , WPVR_LANG ) ,
		'cutv_video_embed_code'        => __( 'Video Embed Code' , WPVR_LANG ) ,
		'cutv_video_service_thumb'     => __( 'Video Thumbnail' , WPVR_LANG ) ,
		'cutv_video_service_duration'  => __( 'Video Duration' , WPVR_LANG ) ,
		'cutv_video_service_views'     => __( 'Video Original Views' , WPVR_LANG ) ,
	);
