<?php
	...
	if( isset( $_GET[ 'save_addon_options' ] ) ) {
		...
		foreach ( $ca[ 'options' ] as $name => $option ) {
	...
	//RESETING OPTIONS
	if( isset( $_GET[ 'reset_addon_options' ] ) ) {
	...
	if( ! is_array( $ca[ 'options' ] ) || count( $ca[ 'options' ] ) == 0 ) $has_options = FALSE;
		...
		foreach ( $addon_options as $key => $row ) {
			$d[ $key ] = $row[ 'order' ];
		}
...
<div class = "wpvr_addon_options_wrapper" addon_id = "<?php echo $addon_id; ?>">
	<div class = "wpvr_addons_header">
		...
		<button
			...
			type = "submit"
			class = "wpvr_save_addon_options pull-right actionBtn wpvr_submit_button"
		>
	...
	<div class = "wpvr_addons_footer">
		...
		<button
			...
			class = "pull-left wpvr_submit_button wpvr_small wpvr_reset_addon_options wpvr_red_button"
		>
