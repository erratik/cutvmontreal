<?php
	...
	if( WPVR_ENABLE_YOUTUBE ) require_once( WPVR_PATH . 'services/youtube/init.php' );
	if( WPVR_ENABLE_VIMEO ) require_once( WPVR_PATH . 'services/vimeo/init.php' );
	if( WPVR_ENABLE_DAILYMOTION ) require_once( WPVR_PATH . 'services/dailymotion/init.php' );
	...
	foreach( (array) $wpvr_vs as $vs ) {
		...
		add_filter(
			...
			function ( $video_fields ) use ( $vs ) {
				if( ! isset( $vs[ 'manual_type' ] ) ) $vs[ 'manual_type' ] = 'text';
				$video_fields[] = array(
					'name'        => $vs[ 'manual_name' ] ,
					'desc'        => $vs[ 'manual_desc' ] ,
					'id'          => $vs[ 'manual_id' ] ,
					...
					foreach( (array) $vs_type[ 'fields' ] as $vs_type_field ) {
						...
						add_filter(
							...
							function ( $video_service_types_fields , $prefix ) use ( $vs , $vs_type , $vs_type_field ) {
								...
								if( ! isset( $field[ 'wpvrClass' ] ) ) $field[ 'wpvrClass' ] = '';
								if( ! isset( $field[ 'wpvr_attributes' ] ) ) $field[ 'wpvr_attributes' ] = array();
