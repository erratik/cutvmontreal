<?php
	...
	function wpvr_adapt_v164wpvr_adapt_v164() {
		update_option( 'wpvr_addons_list' , '' );
		/*wpvr_add_notice( array(
			'slug' => '',
	...
	function wpvr_adapt_v16() {
		...
		foreach ( $sources as $source ) {
			...
			if( $old_postCats == 'false' ) $new_postCats = json_encode( array() );
			else $new_postCats = json_encode( unserialize( $old_postCats ) );
		...
		if( ! isset( $new_wpvr_options ) || ! is_array( $new_wpvr_options ) ) {
			...
			foreach ( $wpvr_default_options as $name => $value ) {
