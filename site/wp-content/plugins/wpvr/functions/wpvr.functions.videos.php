<?php
	...
	if( ! function_exists( 'wpvr_render_modified_player' ) ) {
		function wpvr_render_modified_player( $post_id = null ) {
			...
			if( is_single() ) $auto_play = $wpvr_options[ 'playerAutoPlay' ];
	...
	if( ! function_exists( 'wpvr_add_video' ) ) {
		function wpvr_add_video( $videoItem , $_wpvr_imported = array() , $allowDuplicates = FALSE ) {
			global $wpvr_imported , $wpvr_options , $wpvr_vs;
			...
			if( isset( $wpvr_imported[ $videoItem[ 'service' ] ][ $videoItem[ 'id' ] ] ) ) return FALSE;
			...
			add_post_meta( $newPostId , 'wpvr_video_id' , $videoItem[ 'id' ] , TRUE );
			add_post_meta( $newPostId , 'wpvr_video_sourceName' , $videoItem[ 'sourceName' ] , TRUE );
			add_post_meta( $newPostId , 'wpvr_video_sourceType' , $videoItem[ 'sourceType' ] , TRUE );
			add_post_meta( $newPostId , 'wpvr_video_sourceId' , $videoItem[ 'sourceId' ] , TRUE );

			if( WPVR_ENABLE_POST_FORMATS ) {
				global $wpvr_options;
				set_post_format( $newPostId , $wpvr_options[ 'postFormat' ] );
			}

			if( isset( $videoItem[ 'local_views' ] ) && is_numeric( $videoItem[ 'local_views' ] ) ) {
				add_post_meta( $newPostId , 'wpvr_video_views' , $videoItem[ 'local_views' ] , TRUE );
			} else {
				if( $wpvr_options[ 'startWithServiceViews' ] === TRUE )
					add_post_meta( $newPostId , 'wpvr_video_views' , $videoItem[ 'views' ] , TRUE );
				else
					add_post_meta( $newPostId , 'wpvr_video_views' , 0 , TRUE );
			}
			
			
			add_post_meta( $newPostId , 'wpvr_video_service_views' , $videoItem[ 'views' ] , TRUE );
			add_post_meta( $newPostId , 'wpvr_video_service_url' , $videoItem[ 'url' ] , TRUE );
			add_post_meta( $newPostId , 'wpvr_video_service_thumb' , $videoItem[ 'thumb' ] , TRUE );
			add_post_meta( $newPostId , 'wpvr_video_service_icon' , $videoItem[ 'icon' ] , TRUE );
			add_post_meta( $newPostId , 'wpvr_video_duration' , $videoItem[ 'duration' ] , TRUE );
			add_post_meta( $newPostId , 'wpvr_video_service' , $videoItem[ 'service' ] , TRUE );
			add_post_meta( $newPostId , 'wpvr_video_service_date' , $videoItem[ 'originalPostDate' ] , TRUE );
			add_post_meta( $newPostId , 'wpvr_video_service_desc' , $video_desc , TRUE );
			...
			add_post_meta( $newPostId , 'wpvr_video_embed_code' , $videoItem_embedCode , TRUE );


			add_post_meta( $newPostId , 'wpvr_video_' . $wpvr_vs[ $videoItem[ 'service' ] ][ 'pid' ] . 'Id' , $videoItem[ 'id' ] , TRUE );

			if( WPVR_TAGS_FROM_TITLE_ENABLE ) {
			...
			if( isset( $videoItem[ 'thumb_id' ] ) && $videoItem[ 'thumb_id' ] != '' ) {
				...
				do_action( 'wpvr_event_add_video_thumbnail' , $videoItem , $newPostId , $featured_image_file );
			...
			wpvr_add_log( array(
				...
				"log_msgs" => array(
					...
					'<b>' . __( 'POSTED AS' , WPVR_LANG ) . ' </b>: ' . $video_status ,
					'<b>' . __( 'NEW POST ID' , WPVR_LANG ) . ' </b>: ' . $newPostId ,
				) ,
			) );
			$count_videos = 0;
			foreach ( $wpvr_imported as $s => $service_videos ) {
	...
	if( ! function_exists( 'wpvr_update_imported_videos' ) ) {
		function wpvr_update_imported_videos() {
			...
			if( ! is_array( $wpvr_vs ) || count( $wpvr_vs ) == 0 ) return get_option( 'wpvr_imported' );
		...
		function wpvr_manage_videos( $args = array(), $items = array() ) {
			...
			if( $args[ 'dupsBy' ] != '' ) {
				...
				FROM (  
					SELECT 
						p.*, 
						GROUP_CONCAT(if(m.meta_key = 'wpvr_video_service' , m.meta_value , NULL ) SEPARATOR '') as video_service,
						GROUP_CONCAT(if(m.meta_key = 'wpvr_video_id' , m.meta_value , NULL ) SEPARATOR '') as video_id,
						GROUP_CONCAT(if(m.meta_key = 'wpvr_video_views' , m.meta_value , NULL ) SEPARATOR '') as video_views,
					...
					FROM 
						...
						WHERE 
							m.meta_key IN ('wpvr_video_id','wpvr_video_service' , 'wpvr_video_views')
	...
	if( ! function_exists( 'wpvr_undo_unwant_videos' ) ) {
		function wpvr_undo_unwant_videos( $post_ids ) {
			...
			foreach ( (array) $post_ids as $post_id ) {
				...
				foreach ( $wpvr_unwanted as $k => $unwanted ) {
	...
	if( ! function_exists( 'wpvr_convert_post_to_videoItem' ) ) {
		function wpvr_convert_post_to_videoItem( $post_id , $service = null ) {
			...
			if( ! isset( $postmeta[ 'wpvr_video_service_date' ] ) ) $service_date = '';
			else $service_date = $postmeta[ 'wpvr_video_service_date' ][ 0 ];
			//_d( $thumbs );

			if( $service != null && ! isset( $postmeta[ 'wpvr_video_service' ] ) ) $video_service = $service;
			else $video_service = $postmeta[ 'wpvr_video_service' ][ 0 ];
	...
	if( ! function_exists( 'wpvr_add_videos' ) ) {
		function wpvr_add_videos( $videos ) {
			...
			//global $wpvr_deferred_ids , $wpvr_unwanted_ids  ,$wpvr_deferred , $wpvr_options;
