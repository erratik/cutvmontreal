<?php
	...
	if( isset( $_GET[ 'import_single_video' ] ) ) {
		...
		foreach ( $_POST[ 'items' ] as $k => $item ) {
			...
			//new dBug( $video );
			//return false;
			//$video = json_decode( stripslashes( $item ), true );
	...
	if( isset( $_POST[ 'import_options' ] ) ) {
		...
		$json = (array) json_decode( file_get_contents( $imported_file ) );
	...
	if( isset( $_POST[ 'import_videos' ] ) ) {
		...
		foreach ( $import_videos as $video ) {
