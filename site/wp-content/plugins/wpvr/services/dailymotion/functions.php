<?php
	...
	$vs[ 'get_channel_data' ] = function ( $param = null ) use ( $vs , $pid_suffix ) {
		...
		$api_response = $api_handle->get( $api_endpoint , $api_args );
	...
	$vs[ 'render_subheader' ] = function ( $source = null ) use ( $vs , $pid_suffix ) {
		...
		if( $vs_type[ 'subdata_function' ] === FALSE ) return '';
	...
	$vs[ 'get_single_video_data' ] = function ( $video_id ) use ( $vs , $pid_suffix ) {
		...
		$api_args     = array(
			'fields' => 'created_time,description,tags,thumbnail_120_url,thumbnail_720_url,title,url,views_total,duration' ,
		);
		$api_response = $api_handle->get( $api_endpoint , $api_args );
	...
	$vs_dm[ 'fetch_videos' ] = function ( $videosFound , $options , $oldVideos ) use ( &$vs_dm , $pid_suffix ) {
		...
		if( $videosFound[ 'nextPageToken' ] == '' ) $videosFound[ 'nextPageToken' ] = '1';


		$api_handle = $vs_dm[ 'api_auth' ]();
		if( $api_handle === FALSE ) return $videosFound;


		if( $options[ 'how' ][ 'onlyNewVideos' ] == 'on' ) $options[ 'how' ][ 'onlyNewVideos' ] = TRUE;
		else $options[ 'how' ][ 'onlyNewVideos' ] = FALSE;
		...
		$api_args     = array(
			...
			'limit'  => '100' ,
			'sort'   => 'visited' ,
			'fields' => 'id,created_time,title,url,views_total,description,duration,thumbnail_180_url,thumbnail_720_url,tags' ,

		);


		if( $options[ 'what' ][ 'order' ] == 'title' ) $order = "recent";
		elseif( $options[ 'what' ][ 'order' ] == 'viewCount' ) $order = "visited";
		elseif( $options[ 'what' ][ 'order' ] == 'date' ) $order = "recent";
		elseif( $options[ 'what' ][ 'order' ] == 'relevance' ) $order = "relevance";
		else $order = 'recent';
		...
		if( $options[ 'what' ][ 'mode' ] == 'search' . $pid_suffix ) {
			...
			if( $options[ 'what' ][ $vs_type[ 'param' ] ] == '' ) $options[ 'what' ][ $vs_type[ 'param' ] ] = 'US';
			...
			/* Channel*/
			if( $order == "relevance" ) $order = 'recent';
		...
		if( $options[ 'what' ][ 'videoQuality' ] == 'high' ) $api_args[ 'flags' ] = 'hd';
		...
		if( isset( $api_response[ 'total' ] ) ) $videosFound[ 'totalResults' ] = $api_response[ 'total' ];
		else $videosFound[ 'totalResults' ] = '500000';
		$videosIds                  = "";
	...
	$vs_dmcm[ 'get_comments' ] = function ( $video_id , $post_id , $cData ) use ( &$vs_dmcm , $pid_suffix ) {
		...
		if( ! isset( $api_response[ 'list' ] ) || ! is_array( $api_response[ 'list' ] ) || count( $api_response[ 'list' ] ) == 0 ) return $cData;
